...
public class Sizes implements XRecord{    
    private final String MASTER_TABLE = "Size";
    ...
    private ParamSearchF p_oSearchSizes;
    
    public Sizes(XNautilus foNautilus, String fsBranchCd, boolean fbWithParent){
        p_oNautilus = foNautilus;
        p_sBranchCd = fsBranchCd;
        p_bWithParent = fbWithParent;
        
        p_oSearchSizes = new ParamSearchF(p_oNautilus, ParamSearchF.SearchType.searchSizes);
    ...
    public boolean SaveRecord() {
        ...
        try {
            ...
            if (p_nEditMode == EditMode.ADDNEW){
                ...
                p_oColor.first();
                p_oColor.updateObject("sSizeIDxx", MiscUtil.getNextCode(MASTER_TABLE, "sSizeIDxx", false, loConn, p_sBranchCd));
                p_oColor.updateRow();
                ...
                lsSQL = MiscUtil.rowset2SQL(p_oColor, MASTER_TABLE, "", "sSizeIDxx = " + SQLUtil.toSQL((String) getMaster("sSizeIDxx")));
            }
    ...
    @Override
    public boolean OpenRecord(String fsSizeIDxx) {
        ...
        try {
            if (p_nEditMode != EditMode.UNKNOWN){
                if (p_oColor != null){
                    ...
                    if (p_oColor.getString("sSizeIDxx").equals(fsSizeIDxx)){
            ...
            //open master record
            lsSQL = MiscUtil.addCondition(getSQ_Master(), "sSizeIDxx = " + SQLUtil.toSQL(fsSizeIDxx));
            loRS = p_oNautilus.executeQuery(lsSQL);
            p_oColor = factory.createCachedRowSet();
            p_oColor.populate(loRS);
            MiscUtil.close(loRS);
    ...
    private void initMaster() throws SQLException{
        ...
        p_oColor.updateObject("sSizeIDxx", MiscUtil.getNextCode(MASTER_TABLE, "sSizeIDxx", false, p_oNautilus.getConnection().getConnection(), p_sBranchCd));
    ...
    private boolean isEntryOK(){
        try {
            ...
            if (String.valueOf(getMaster("sSizeName")).isEmpty()){
    ...
    public JSONObject searchSizes(String fsKey, Object foValue, boolean fbExact){
        p_oSearchSizes.setKey(fsKey);
        p_oSearchSizes.setValue(foValue);
        p_oSearchSizes.setExact(fbExact);
        
        return p_oSearchSizes.Search();
    }
    
    public ParamSearchF getSearchSizes(){
        return p_oSearchSizes;
    }
}
